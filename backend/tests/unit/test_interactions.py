...
def test_filter_returns_multiple_matches_when_several_share_item_id() -> None:
    ...
    result = filter_by_item_id(interactions, 5)
    assert len(result) == 3
    assert all(i.item_id == 5 for i in result)


def test_filter_returns_empty_when_no_matching_item_id() -> None:
    """Edge case: filtering by non-existent item_id returns empty list."""
    interactions = [_make_log(1, 1, 1), _make_log(2, 2, 2)]
    result = filter_by_item_id(interactions, 999)
    assert result == []
